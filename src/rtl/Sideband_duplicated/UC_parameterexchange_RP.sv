// ================================================================================================================================
//  FILENAME    : UC_parameterexchange_RP.sv
//  MODULE      : UC_parameterexchange_RP
...
parameter TIMEOUT_CYCLES = 100;
    typedef enum bit [2:0] { 
        ...
        ST_SEND_CXL_ADV_CAP    = 3'b011,
        ST_WAIT_CXL_ADV_CAP    = 3'b010,
        ST_SUCCESS_EXCHANGE    = 3'b110,
        ST_INVALID_EXCHANGE    = 3'b111,
        ST_TIMEOUT             = 3'b101
    } paramexchange_state;
 //===================================================== SIGNALS ====================================================
...
logic        s_reserved_ok;
logic        s_pcie_valid;
logic        s_rp_cxl_valid;
// For RP: validate the common capability
    assign s_pcie_cap_to_check = s_common_adapter_cap;
...
assign s_rp_cxl_valid = s_common_cxl_cap[0] && ({s_common_cxl_cap[17:14],s_common_cxl_cap[12:8], s_common_cxl_cap[4:1]} == 13'b0);
 ...
 always_comb begin : proc_next_state
    ...
    case (r_current_state)
        ...
        ST_WAIT_ADAPTER_CAP: begin
            if (!s_param_exch_timeout_hit) begin
                if (i_rx_msg_valid && (s_Msgcode == 8'h01) && (s_MsgSubcode == 8'h00) && (s_MsgInfo == 16'h0)) begin
                    ...
                    else if (s_pcie_valid)
                        r_next_state = ST_SEND_CXL_ADV_CAP;
                    else
                        r_next_state = ST_INVALID_EXCHANGE;
                end
                else
        ...
        ST_SEND_CXL_ADV_CAP: begin
            if (!s_param_exch_timeout_hit)
 ...
 always_comb begin : proc_output_logic
    ...
    case (r_current_state)
        ST_IDLE: begin
            if (i_start_PE) begin
                // Send the Adv Cap for non-streaming protocol (RP)
            ...
            if (!s_param_exch_timeout_hit) begin
                if (i_rx_msg_valid && (s_Msgcode == 8'h01) && (s_MsgSubcode == 8'h00) && (s_MsgInfo == 16'h0)) begin
                    if (s_streaming_mode || (s_rx_adapter_advcap[4:1] == 4'b1000)) begin
                        ...
                        if (s_pcie_valid) begin
                            case ({i_format6_enabled, i_format4_enabled})
                                ...
                                2'b11: begin
                                    if (s_common_adapter_cap[27])
                                        o_flit_fromat_status = 4'b0110; // Format 6
                                    else if (s_common_adapter_cap[25])
                                        o_flit_fromat_status = 4'b0100; // Format 4
                                    else if (s_common_adapter_cap[24])
                                        o_flit_fromat_status = 4'b0011; // Format 3
                                    else
                        ...
                        // Send the Finalized Cap even if it's not correct to make the remote device also decide invalid Parameter exchange
                        o_tx_msg_with_data     = {s_common_adapter_cap, ^s_common_adapter_cap, 63'h05000000_2000801b};
                        o_tx_msg_valid         = 1'b1;
                        o_adapter_fincap       = s_common_adapter_cap;
        ...
        ST_SEND_CXL_ADV_CAP: begin
            if (!s_param_exch_timeout_hit) begin
                // As a RP, send my adv_cap.cxl to the remote partner
        ...
        ST_WAIT_CXL_ADV_CAP: begin
            if (!s_param_exch_timeout_hit) begin
                if (i_rx_msg_valid && (s_Msgcode == 8'h01) && (s_MsgSubcode == 8'h01) && (s_MsgInfo == 16'h0)) begin
                    // RP: Send the Fin cap.cxl after receiving the remote adv cap.cxl
                    o_tx_msg_with_data = {i_cxl_advcap & s_rx_cxl_advcap, ^(i_cxl_advcap & s_rx_cxl_advcap), 63'h45000001_2000801b};
                    o_tx_msg_valid     = 1'b1;
                    o_cxl_fincap       = (i_cxl_advcap & s_rx_cxl_advcap);
