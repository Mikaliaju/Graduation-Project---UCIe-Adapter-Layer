...
module replay_schedule (
    ...
    input  logic [7:0] i_received_valid_seq_num,
    ...
    if(!rst_n) begin
        ...
        if(i_replay_timeout_flit_count >= 375) begin
            ...
            if(!i_replay_in_progress && !o_replay_scheduled) begin //a payload flit with flit sequence number n + 1 is stored in the tx retry buffer.
                if(i_nak_ignore_flit_seq_num != i_received_valid_seq_num || i_nak_ignore_flit_seq_num == 0)
                    replay_schedule_1();     
            end
        end
        else if(i_replay_in_progress) begin
            o_replay_scheduled         <= 1'b0;
            o_start_buffer_replay_mode <= 1'b0;
        end
    end
...
task replay_schedule_1();
    o_replay_scheduled <= 1'b1;
    o_replay_scheduled_type <= standard_replay;
    o_start_buffer_replay_mode <= 1'b1;
    o_tx_replay_flit_seq_num <= i_received_valid_seq_num + 1;
    o_nak_ignore_flit_seq_num <= i_received_valid_seq_num;
endtask
