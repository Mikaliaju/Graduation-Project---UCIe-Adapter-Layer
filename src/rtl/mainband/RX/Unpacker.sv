    ...
    case (r_state)
      ...
      S_RECEIVE: begin
        if (i_stop_stream || !i_unpacker_en) begin
          ...
          // Feed chunk to CRC_Generator
          if (r_chunk_cnt == 2'd3) begin
            // Chunk 3: masked (zero out CRC fields)
            ...
            r_pid        <= i_pl_data_rdi[C3_FH_B0+7 : C3_FH_B0+6];
            r_sid        <= i_pl_data_rdi[C3_FH_B0+5];
            r_dllp_ofc   <= i_pl_data_rdi[C3_FH_B0+4];
            r_replay_cmd <= i_pl_data_rdi[C3_FH_B1+5 : C3_FH_B1+4];

            // Sequence number: upper=FH_B0[3:0], lower=FH_B1[3:0]
            r_seq_num    <= {i_pl_data_rdi[C3_FH_B0+3 : C3_FH_B0],i_pl_data_rdi[C3_FH_B1+3 : C3_FH_B1]};

            // DLP bytes
            r_dllp_buf      <= i_pl_data_rdi[C3_DLP  +: 32];

            // Received CRC values (for comparison in S_CHECK)
            r_crc0_ch       <= i_pl_data_rdi[C3_CRC0 +: 16];
            r_crc1_ch       <= i_pl_data_rdi[C3_CRC1 +: 16];

            // Forward DLLP info to FDI
            o_pl_dllp_valid <= 1'b1;
            o_pl_dllp_ofc   <= i_pl_data_rdi[C3_FH_B0+4];
      ...
      // =====================================================================
      // S_CHECK : Cycle 5
      //   - CRC_Generator result ready (w_crc_valid asserted)
