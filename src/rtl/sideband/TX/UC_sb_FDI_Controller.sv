...
typedef enum logic [3:0] {
    S_IDLE       = 4'd0,
    S_POP        = 4'd1,   // assert o_Rd_en
    S_POP_WAIT   = 4'd2,   // wait 1 cycle for data to appear on i_Data_out
    S_PARSE      = 4'd3,   // parity check + routing decision
    S_SEND_PHY   = 4'd4,   // forward packet to RDI FIFO (stall if full)
    S_ISSUE_LOCAL= 4'd5,   // present request to Access Arbiter (first cycle)
    S_WAIT_LOCAL = 4'd6,   // hold request until i_Local_done
    S_BUILD_COMP = 4'd7,   // build completion packet (registered)
    S_PUSH_COMP  = 4'd8    // output completion for one cycle
} state_t;
        ...
        case (r_state)
            ...
            S_BUILD_COMP: begin
                begin : build_block
                    ...
                    // Start from the original request as the template
                    cmp = r_req;

                    // Header modifications
                    // Replace opcode with completion opcode
                    cmp[4:0]   = r_comp_opcode;

                    // Completion status goes into bits [34:32]
                    // (p1 bits [2:0] → r_req[34:32])
                    cmp[34:32] = i_Local_status;

                    //  Payload
                    if (r_read_req) begin
                        // Read completion: return register data
                        if (r_is_32b) begin
                            cmp[95:64]  = i_Local_R_data[31:0];
                            cmp[127:96] = 32'h0;
                        end
                        else begin
                            cmp[95:64]  = i_Local_R_data[31:0];
                            cmp[127:96] = i_Local_R_data[63:32];
                        end
                    end
                    else begin
                        // Write completion: zero payload
                        cmp[127:64] = 64'h0;
                    end

                    // Recompute parity 
                    // DP = XOR over data word
                    cmp[63] = ^cmp[127:64];
                    // CP = XOR over header bits excluding dp[63] and cp[62]
                    cmp[62] = ^{cmp[61:32], cmp[31:0]};
